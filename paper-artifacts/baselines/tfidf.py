...
-----------------
  ...
  When fit_corpus = 'all_instances': fit on ALL author_instances (≈183 k texts).
  When fit_corpus = 'eval_pairs_only': fit on the 2 × N unique eval texts only.

AIN vectorisation
-----------------
  Text = affil_norm (normalised affiliation string).
  When fit_corpus = 'all_instances': fit on ALL affil_instances (≈173 k texts).
...
from vs2.eval.baselines.features import strip_author_id
