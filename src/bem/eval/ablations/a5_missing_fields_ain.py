"""a5_missing_fields_ain.py -- Ablation A5: AIN missing-field ablations.

Measures the contribution of individual evidence components to BEM AIN
matching performance by calling the LLM with a modified evidence card.

Variants:
  raw_affil_only     -- only affil_raw / affil_norm / affil_acronyms included;
                       linked_authors_norm set to empty
  remove_author_link -- same as full card but linked_authors_norm removed

Fairness:
  title / source / year are NEVER included (same constraint as full BEM).

Same skip/force / confirmation / C6 guard behaviour as A4.
"""
...
from bem.llm_verify.evidence_cards import build_ain_evidence


_VARIANTS = {
    "raw_affil_only":     "Raw affiliation string only (no author-link context)",
    "remove_author_link": "No linked-author context",
}
...
def run_a5(
    ...
    """Run ablation A5 for each requested variant."""
    api_key = os.environ.get("ANTHROPIC_API_KEY", "")
    if not api_key:
        raise EnvironmentError("[A5] ANTHROPIC_API_KEY is not set.")

    try:
        import anthropic
        client = anthropic.Anthropic(api_key=api_key)
    except ImportError:
        raise ImportError("[A5] Install 'anthropic': pip install anthropic")

    thresholds = _load_thresholds(thresholds_manifest, "ain")

    bm_df    = pd.read_parquet(benchmark_ain)
    inst_df  = pd.read_parquet(affil_instances)
    auth_df  = pd.read_parquet(author_instances)
    rec_df   = pd.read_parquet(records_normalised)
    prompt_template = _load_prompt(ain_prompt_path)
    ...
    for variant in variants:
        if variant not in _VARIANTS:
            print(f"  [A5] unknown variant '{variant}' -- skipping")
            continue

        cache_path     = (output_dir / f"metrics_a5_{variant}.json") if output_dir else None
        decisions_path = (output_dir / f"a5_decisions_{variant}.jsonl") if output_dir else None

        if cache_path and cache_path.exists() and not force:
            print(f"  [A5/{variant}] loading cached results")
            results[variant] = json.loads(cache_path.read_text(encoding="utf-8"))
            continue

        print(f"\n  [A5/{variant}] {_VARIANTS[variant]}")
        print(f"    Pairs  : {n_sample}")
        print(f"    Model  : {model}")

        if require_confirmation and sys.stdin.isatty():
            ans = input(f"  Continue with A5/{variant}? [y/N] ").strip().lower()
            if ans != "y":
                print(f"  [A5/{variant}] skipped by user.")
                continue
        elif require_confirmation and not sys.stdin.isatty():
            print(f"  [A5/{variant}] non-interactive -- skipping.")
            continue
        ...
        guarded = apply_guards(decisions, task="AIN", thresholds=thresholds, m_signals=_M_SIGNALS_DEFAULT)
        ...
        variant_result = [{
            "ablation":    "a5_missing_fields_ain",
            "variant":     variant,
            "task":        "ain",
...
def _run_variant(
    ...
    existing     = _load_existing(decisions_path, force)
    records_out  = []
    last_call    = 0.0
    ...
    try:
        for _, pair in sample.iterrows():
            ...
            if evidence is None:
                records_out.append(_stub(pair, "build_error"))
                continue

            filled = prompt_template.replace("{{EVIDENCE_CARD}}", json.dumps(evidence, ensure_ascii=False))
            ...
            try:
                ...
                print(f"    [A5/{variant}] API error: {exc}")
...
def _build_modified_evidence(
    ...
    try:
        ...
        print(f"    [A5] evidence build error for {anchor_id}: {exc}")
        return None

    if variant in ("raw_affil_only", "remove_author_link"):
        for side in ("anchor", "candidate"):
            evidence[side]["linked_authors_norm"] = []
            evidence[side]["linked_authors_fallback"] = True

    return evidence


# ---------------------------------------------------------------------------
# Helpers (shared with A4)
# ---------------------------------------------------------------------------

def _load_prompt(path: Path) -> str:
    if not path.exists():
        raise FileNotFoundError(f"AIN prompt not found: {path}")
...
def _load_existing(path: Path | None, force: bool) -> dict:
    ...
    with open(path, encoding="utf-8") as fh:
        for line in fh:
            obj = json.loads(line)
            out[(obj["anchor_id"], obj["candidate_id"])] = obj
    return out


def _stub(pair: pd.Series, reason: str) -> dict:
    return {
        "anchor_id":    str(pair["anchor_id"]),
        "candidate_id": str(pair["candidate_id"]),
        "gold_label":   str(pair.get("gold_label", "")),
